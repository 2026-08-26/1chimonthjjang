...
def dataset_meta(path, df):
    ...
    date_col = next((c for c in ['Date','game_date','일시','date'] if c in cols), None)
...
def _build(_signature):
    ...
    for path in ['result/all_social_signals.csv','result/all_economy_signals.csv','result/stock/all_stock_signals.csv']:
    ...
    return dict(
        overview=dict(domains=sum([len(social)>0,len(economy)>0,len(stock)>0,len(baseball)>0,len(content)>0]),raw=sum(s['file'].startswith('data/raw/') for s in sources),processed=sum(s['file'].startswith('data/processed/') for s in sources),total=len(signals),levels=counts,domains_detail=domain_counts),
        ...
        stock=dict(period=period(stock,'date'),rows=len(stock),latest=latest,latest_count=len(latest_stock),data=records(latest_stock),thresholds='거래대금 10억 원 이상 · 최근 21개 관측치 내 무거래/상장주식수 변동 제외. 거래량 ≥ 직전 20일 평균 3배 · |5거래일 수익률| ≥ 10% · 거래대금 점유율 ≥ 평균 3배이면서 시장 내 0.1% 이상. HIGH/MEDIUM 등급은 원 코드에 없음.'),
        ...
        limitations=['Apart Deal.csv 원본이 없어 economy_monthly.csv의 원본 재현은 불가합니다.','stock_daily.csv와 KRX 원본 CSV가 없어 주식 차트는 시그널 파일에 포함된 종목만 비교합니다. stock_meta.json의 전체 행 수는 KPI에 사용하지 않았습니다.','주식은 등급 미분류, 야구는 기존 BaseballAIAgent의 강도 기준(0.8 / 0.5)을 적용합니다. 분야 간 점수·발생 건수는 동일 척도가 아닙니다.','K콘텐츠 관심도는 기존 엔진의 고정 시드 시뮬레이션이며 실제 관측 시그널 합계에서 제외합니다.'])
